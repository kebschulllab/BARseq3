...
def call_genes_tiled_with_overlap(Xflat, codeflat, tile_size=1024, overlap=0, iterations=100, thresh_refined = None):
	import bardensr
	...
	print("Xflat dtype:", np.asarray(Xflat).dtype)
	print("codeflat.dtype:", np.array(codeflat).dtype)
		...
		for j in range(0, W, tile_size):
			...
			try:
				et_tile, _ = bardensr.spot_calling.estimate_density_iterative(
					Xtile, codeflat, iterations=iterations, rounds = len(round_num), estimate_phasing = True)
				et_tile = et_tile.squeeze(axis=0)  # Remove batch dimension
			except Exception as e:
				print(f"Tile ({i}:{i_end}, {j}:{j_end}) failed: {e}")
				continue
			print("turned on estimate phasing and included rounds")
...
def call_genes(threshold, noisefloor, rounds_all, codeflat, genenames, positions_reg, poolsize, xdim, ydim, dim):
    ...
    for i in range(len(rounds_all)):
        Xflat = rounds_all[i].reshape((R*C,)+rounds_all[i].shape[-2:])
        Xflat = np.expand_dims(Xflat,axis=1)
        print("hey girl you changed it")
        et = call_genes_tiled_with_overlap(Xflat, codeflat, tile_size=512, overlap=0,iterations=100)
        # et = bardensr.spot_calling.estimate_density_singleshot(Xflat, codeflat, noisefloor)
        genes_result_sing = bardensr.spot_calling.find_peaks(et,threshold, poolsize)# , use_tqdm_notebook=True

        et_squeezed = et.squeeze(0)
        pixel_values = genes_result_sing.apply(lambda row: et_squeezed[int(row['m2']), int(row['m1']), int(row['j'])], axis=1) 
        genes_result_sing['pixel_val'] = pixel_values
        max_j_array = np.argmax(et_squeezed, axis=2)
        
        genes_result_sing['is_max_j'] = genes_result_sing.apply(
			lambda row: row['j'] == max_j_array[int(row['m1']), int(row['m2'])],axis=1)
        genes_result_sing = genes_result_sing[genes_result_sing['is_max_j']].drop(columns='is_max_j')
        print("hey girl you changed it")
...
def find_peaks_in_image(image, codeflat, thresh_refined, noisefloor, len_wid, FOV):
     ...
     for i in range(len(patches)):
        ...
        # et = call_genes_tiled_with_overlap(Xflat, codeflat, tile_size=512, overlap=0,iterations=100, thresh_refined=thresh_refined)
        # spots=bardensr.spot_calling.find_peaks(et,thresh_refined,poolsize=(0,1,1), use_tqdm_notebook=False)
        # print("changed this")
        # et_squeezed = et.squeeze(0)  
